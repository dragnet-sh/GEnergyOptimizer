...
extension ZoneListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        //ToDo: Safely UnWrap
        if let zoneList = state.getPFAudit()?.zoneCollection[activeEZone.rawValue] {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let auditDTO = state.getPFAudit() {
            if let zoneCollection = auditDTO.zoneCollection[self.activeEZone.rawValue] {
                if let zoneDTO = zoneCollection[indexPath.row] as? PFZone {
                    ...
                    state.getZone(objectId: objectId) { status, pfObject in
                        ...
                        featureViewController!.zoneDTO = pfObject as! PFZone //ToDo: Forced Unwrapping
